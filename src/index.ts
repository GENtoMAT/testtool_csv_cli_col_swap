...
async function main() {
  ...
  //console.log("raw detect:" + Encoding.detect(csvRawReaded))//エンコードを検出
  const aaa = Encoding.convert(csvRawReaded, {
    from: "SJIS",
    to: "UNICODE",
    type: "string",
  })

  const records = parse(aaa, {
  ...
  // 文字列をShift_JISに変換する
  const sjisBytes = Encoding.convert(csvSwaped, {
    from: "UNICODE",
    to: "UTF8", //shiftJISなら "SJIS"
    type: "arraybuffer",
  })
  //入れ替え済みのCSV形式文字列を出力
  fs.writeFileSync(outputFilePath, Buffer.from(sjisBytes))
